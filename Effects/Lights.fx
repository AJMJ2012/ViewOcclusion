...
uniform texture shadowMapHolesTexture;
sampler shadowMapHoles = sampler_state {
	Texture = <shadowMapHolesTexture>;
	MipFilter = Point;
	MinFilter = Point;
	MagFilter = Point;
	AddressU = Clamp;
...
float4 CompositeFinal(float2 coords : TEXCOORD0) : COLOR0 {
	...
	for (float d = 0.0; d < Pi2; d += Pi2 / 6) { {or (float i = 0.3333; i <= 1.0; i += 0.3333)
		{
	...
	color.xyz += tex2D(shadowMapHoles, coords).xyz;

	return color;
}


technique Shadowcasting { {ass DistanceToShadowcaster
	{
